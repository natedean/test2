...
function MainController(QuestionFactory, $timeout){
  ...
  vm.categories = ['All', 'Guitar', 'Music Theory','Sight Reading'];
  vm.selectedCategories = ['All', 'Guitar', 'Music Theory', 'Sight Reading'];
  ...
  function selectCategory(e){
    let selectionAttempt = e.target.innerText;
    if (selectionAttempt === 'All' && (vm.selectedCategories.length === vm.categories.length)){
      return;
    }
    else if (_.includes(vm.selectedCategories, selectionAttempt) && selectionAttempt){
      _.remove(vm.selectedCategories, (item)=>{ return item === selectionAttempt || item === 'All' ; });
    }else{
      vm.selectedCategories.push(selectionAttempt);
    }
    if (_.isEmpty(vm.selectedCategories) || (selectionAttempt !== 'All' && vm.selectedCategories.length >= (vm.categories.length - 1))){
